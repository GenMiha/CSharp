    ...
    public partial class FileManager : ServiceBase
    {
        ...
        protected override async void OnStart(string[] args)
        {
            ...
            if (File.Exists(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "options.xml")) &&
                File.Exists(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "options.xsd")))
            {
                XmlSchemaSet schema = new XmlSchemaSet();

                schema.Add(string.Empty, Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "options.xsd"));

                XDocument xdoc = XDocument.Load(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "options.xml"));
